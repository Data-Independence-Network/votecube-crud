...
var (
	emailAddressColumns               = []string{"email_name", "email_domain_id", "user_account_id", "created_at"}
	emailAddressColumnsWithoutDefault = []string{"email_name", "email_domain_id", "user_account_id", "created_at"}
	emailAddressColumnsWithDefault    = []string{}
	emailAddressPrimaryKeyColumns     = []string{"email_name", "email_domain_id"}
)
...
func (o *EmailAddress) EmailDomain(mods ...qm.QueryMod) emailDomainQuery {
	queryMods := []qm.QueryMod{
		qm.Where("email_domain_id=?", o.EmailDomainID),
	}
...
func (emailAddressL) LoadUserAccount(ctx context.Context, e boil.ContextExecutor, singular bool, maybeEmailAddress interface{}, mods queries.Applicator) error {
	var slice []*EmailAddress
	var object *EmailAddress

	if singular {
		object = maybeEmailAddress.(*EmailAddress)
	} else {
		slice = *maybeEmailAddress.(*[]*EmailAddress)
	}

	args := make([]interface{}, 0, 1)
	if singular {
		if object.R == nil {
			object.R = &emailAddressR{}
		}
	...
	if singular {
		...
		foreign.R.EmailAddresses = append(foreign.R.EmailAddresses, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserAccountID == foreign.UserAccountID {
				...
				foreign.R.EmailAddresses = append(foreign.R.EmailAddresses, local)
				break
			}
		}
	}
...
// LoadEmailDomain allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (emailAddressL) LoadEmailDomain(ctx context.Context, e boil.ContextExecutor, singular bool, maybeEmailAddress interface{}, mods queries.Applicator) error {
	var slice []*EmailAddress
	var object *EmailAddress
	...
	query := NewQuery(qm.From(`email_domain`), qm.WhereIn(`email_domain_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load EmailDomain")
	}

	var resultSlice []*EmailDomain
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice EmailDomain")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for email_domain")
	}
	if err = results.Err(); err != nil {
...
func (o *EmailAddress) SetUserAccount(ctx context.Context, exec boil.ContextExecutor, insert bool, related *UserAccount) error {
	...
	values := []interface{}{related.UserAccountID, o.EmailName, o.EmailDomainID}
	...
	if related.R == nil {
		related.R = &userAccountR{
			EmailAddresses: EmailAddressSlice{o},
		}
	} else {
		related.R.EmailAddresses = append(related.R.EmailAddresses, o)
	}
...
func (o *EmailAddress) SetEmailDomain(ctx context.Context, exec boil.ContextExecutor, insert bool, related *EmailDomain) error {
	...
	values := []interface{}{related.EmailDomainID, o.EmailName, o.EmailDomainID}
	...
	o.EmailDomainID = related.EmailDomainID
	if o.R == nil {
		o.R = &emailAddressR{
			EmailDomain: related,
		}
	} else {
		o.R.EmailDomain = related
	}

	if related.R == nil {
		related.R = &emailDomainR{
			EmailAddresses: EmailAddressSlice{o},
		}
	} else {
		related.R.EmailAddresses = append(related.R.EmailAddresses, o)
...
func FindEmailAddress(ctx context.Context, exec boil.ContextExecutor, emailName string, emailDomainID int64, selectCols ...string) (*EmailAddress, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"email_address\" where \"email_name\"=$1 AND \"email_domain_id\"=$2", sel,
	)

	q := queries.Raw(query, emailName, emailDomainID)
...
func (o *EmailAddress) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryCockroachDB(dialect, "\"email_address\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *EmailAddress) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"email_address\" WHERE \"email_name\"=$1 AND \"email_domain_id\"=$2"
...
func (o *EmailAddress) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindEmailAddress(ctx, exec, o.EmailName, o.EmailDomainID)
	if err != nil {
		return err
	}
...
func EmailAddressExists(ctx context.Context, exec boil.ContextExecutor, emailName string, emailDomainID int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"email_address\" where \"email_name\"=$1 AND \"email_domain_id\"=$2 limit 1)"

	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, sql)
		fmt.Fprintln(boil.DebugWriter, emailName, emailDomainID)
	}

	row := exec.QueryRowContext(ctx, sql, emailName, emailDomainID)
