...
func testAddressToManyUserPersonalInfos(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadUserPersonalInfos(ctx, tx, false, (*[]*Address)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAddressToManyAddOpUserPersonalInfos(t *testing.T) {
	var err error
...
func testAddressToOneStreetUsingStreet(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, streetDBTypes, false, streetColumnsWithDefault...); err != nil {
	...
	local.StreetID = foreign.StreetID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Street().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}

	if check.StreetID != foreign.StreetID {
		t.Errorf("want: %v, got %v", foreign.StreetID, check.StreetID)
	}

	slice := AddressSlice{&local}
	if err = local.L.LoadStreet(ctx, tx, false, (*[]*Address)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Street == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Street = nil
	if err = local.L.LoadStreet(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Street == nil {
		t.Error("struct should have been eager loaded")
...
func testAddressToOneSetOpStreetUsingStreet(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, streetDBTypes, false, strmangle.SetComplement(streetPrimaryKeyColumns, streetColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, streetDBTypes, false, strmangle.SetComplement(streetPrimaryKeyColumns, streetColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Street{&b, &c} {
		err = a.SetStreet(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.StreetID != x.StreetID {
			t.Error("foreign key was wrong value", a.StreetID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.StreetID))
		reflect.Indirect(reflect.ValueOf(&a.StreetID)).Set(zero)
		...
		if a.StreetID != x.StreetID {
			t.Error("foreign key was wrong value", a.StreetID, x.StreetID)
		}
	}
}
...
var (
	addressDBTypes = map[string]string{`AddressID`: `int8`, `BuildingUnit`: `varchar`, `IsBuildingUnit`: `bool`, `PostCode`: `varchar`, `StreetID`: `int8`, `StreetNumber`: `int2`}
	_              = bytes.MinRead
)
