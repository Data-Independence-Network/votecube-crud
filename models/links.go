...
var (
	linkColumns               = []string{"link_id", "link_server", "link_addres", "user_account_id", "created_at"}
	linkColumnsWithoutDefault = []string{"link_id", "link_server", "link_addres", "user_account_id", "created_at"}
	linkColumnsWithDefault    = []string{}
	linkPrimaryKeyColumns     = []string{"link_id"}
)
...
func (o *Link) MessagesLinks(mods ...qm.QueryMod) messagesLinkQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"messages_links\".\"link_id\"=?", o.LinkID),
	)
...
func (o *Link) PollsLinks(mods ...qm.QueryMod) pollsLinkQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"polls_links\".\"link_id\"=?", o.LinkID),
	)
...
func (linkL) LoadUserAccount(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLink interface{}, mods queries.Applicator) error {
	var slice []*Link
	var object *Link
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (linkL) LoadMessagesLinks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLink interface{}, mods queries.Applicator) error {
	var slice []*Link
	var object *Link
	...
	query := NewQuery(qm.From(`messages_links`), qm.WhereIn(`link_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (linkL) LoadPollsLinks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLink interface{}, mods queries.Applicator) error {
	var slice []*Link
	var object *Link
	...
	query := NewQuery(qm.From(`polls_links`), qm.WhereIn(`link_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Link) SetUserAccount(ctx context.Context, exec boil.ContextExecutor, insert bool, related *UserAccount) error {
	...
	values := []interface{}{related.UserAccountID, o.LinkID}
...
func (o *Link) AddMessagesLinks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*MessagesLink) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.LinkID, rel.MessageLinkID}
...
func (o *Link) AddPollsLinks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PollsLink) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.LinkID, rel.PollLinkID}
...
func FindLink(ctx context.Context, exec boil.ContextExecutor, linkID int64, selectCols ...string) (*Link, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"links\" where \"link_id\"=$1", sel,
	)
...
func (o *Link) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryCockroachDB(dialect, "\"links\"", updateOnConflict, ret, update, conflict, insert)
...
func LinkExists(ctx context.Context, exec boil.ContextExecutor, linkID int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"links\" where \"link_id\"=$1 limit 1)"
